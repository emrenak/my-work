...
public class dbApp {
	...
	static String dbReadBegin = "Air response code :0";
	static String dbReadEnd = "db catalog information retrieved";
	public static void main(String[] args) {
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss,SSS"); 
		try (BufferedReader br = new BufferedReader(new FileReader("dbTimes.out"))) {
			...
			while ((sCurrentLine = br.readLine()) != null) {
				...
				if(!msisdn.equals(prevMsisdn)){
					if(sCurrentLine.contains(dbReadBegin)){
						// start time
					...
					if(!diffCalculated && sCurrentLine.contains(dbReadEnd)){
			...
			System.out.println("Total processing for db: " + totalProcessing +  " millis. Total msisdn size: " + totalMsisdnProcessed);
